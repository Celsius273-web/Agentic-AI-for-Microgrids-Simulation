...
- All systems operating within normal parameters"""
                ...
                for word in ["problem", "issue", "alert", "emergency"]
            ):
    ...
    async def _researcher_response(self, message: str) -> str:
        ...
        if rag_result["status"] == "success" and rag_result["results"]:
            ...
            for result in rag_result["results"]:
                findings.append(
                    f"**{result['metadata']['section_header']}** (from {result['metadata']['source_file']}):\n{result['content']}"
                )
    ...
    async def chat_endpoint(request: ChatRequest) -> ChatResponse:
        ...
        try:
            if request.agent not in agents:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid agent: {request.agent}. Available: {list(agents.keys())}",
                )
            ...
            raise HTTPException(
                status_code=500, detail=f"Agent communication failed: {str(e)}"
            )
...
def run_demo_server(port: int = 8002):
