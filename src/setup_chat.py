...
def test_rag_system():
    ...
    docs_dir = str(SRC_ROOT / "shared" / "docs")
    ...
    for query in test_queries:
        ...
        if search_result["status"] == "success":
            ...
            for result in search_result["results"][:1]:
...
def check_file_structure():
    ...
    required_paths = [
        (SRC_ROOT / "shared/docs", "Documentation directory"),
        (SRC_ROOT / "shared/docs/grid_standards.md", "Grid standards documentation"),
        (SRC_ROOT / "shared/docs/best_practices.md", "Best practices documentation"),
        (SRC_ROOT / "shared/docs/past_decisions.md", "Past decisions documentation"),
        (SRC_ROOT / "microgrid_ui", "React UI directory"),
        (SRC_ROOT / "microgrid_ui/src/ChatInterface.jsx", "Chat interface component"),
        (SRC_ROOT / "chat_server.py", "Chat server implementation"),
        (SRC_ROOT / "shared/rag.py", "RAG system implementation"),
        (REPO_ROOT / "microgrid_agent.py", "Microgrid agent"),
    ]
    ...
    for path, description in required_paths:
        if path.exists():
            print(f"✅ {path.relative_to(REPO_ROOT)} - {description}")
        else:
...
def main():
    ...
    print("   python3 src/chat_server.py")
    print("\n2. Start the React UI:")
    print("   cd src/microgrid_ui")
    print("   npm install")
    print("   npm start")
